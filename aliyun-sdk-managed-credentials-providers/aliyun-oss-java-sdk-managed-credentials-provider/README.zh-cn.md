...
## ⚠️ 注意事项：OSS上传InputStream重试限制

当使用InputStream进行OSS文件上传时，请注意以下限制：

### 问题说明
在凭证刷新重试场景下，系统需要重新读取InputStream数据。如果流不支持`mark()`/`reset()`操作，将无法完成重试。

**注意**：自动轮转不会出现此问题，手动轮转时才可能遇到。推荐使用自动轮转。
...
| 流类型 | 重试支持 | 说明 |
|--------|----------|------|
| FileInputStream | ✅ | 推荐使用 |
| ByteArrayInputStream | ✅ | 适合小数据 |
| BufferedInputStream | ⚠️ | 需预先mark |
| 网络流 | ❌ | 不支持重试 |

### 最佳实践
1. 优先使用File对象而非InputStream
2. 大文件考虑先保存为临时文件
3. 避免使用一次性网络流进行重要上传操作
